...
loadMoney();
$("#ardoraTab").css("visibility","hidden");
...
var newHtml="";if ($.trim(im[ram_G[indexGame]])){newHtml='<img id="imaL" class="imaLeft" alt="'+imalt[ram_G[indexGame]]+'" src="LO_SCONTRINO_resources/media/'+im[ram_G[indexGame]]+'">';}
newHtml=newHtml+te[ram_G[indexGame]];$("#actText").html(newHtml);
$("#imaL").attr("tabindex","0");if ($.trim(im[ram_G[indexGame]])) {$( "#imaL" ).css("cursor","pointer");$( "#imaL" ).click(function() {imaUp("LO_SCONTRINO_resources/media/G" + im[ram_G[indexGame]],imalt[ram_G[indexGame]])});
$("#imaL").keydown(function(e){if (e.which!=9){ imaUp("LO_SCONTRINO_resources/media/G" + im[ram_G[indexGame]],imalt[ram_G[indexGame]])}});
}
$("#actText").css("minHeight",imaH[ram_G[indexGame]]);$(".buttonAu").remove();
if ($.trim(a1[ram_G[indexGame]])){$("#origin").append('<img alt="Play" class="buttonAu" src="LO_SCONTRINO_resources/media/sound.png"/>');
...
for (i = values.length - 1; i > -1; i--) {while (avMoney[i] > 0 && values[i] + totPaint <= tot) {var newCoin = $('<img class="coin" src="LO_SCONTRINO_resources/media/'+m[i].substr(1)+'.gif"/>');
newCoins.push(newCoin);totPaint = totPaint + values[i];avMoney[i]--;}}for (i = 0; i < newCoins.length; i++) {$("#drop").append(newCoins[i]);}
}
function paintBack(){}
